...
pair<Poly, Poly> solve(int l, int r, Poly F) {
  if (l == r) {
    Poly mul(p[l]), res = (F * mul).divxk(m);
    for (Z x : res.a) ans[l] += x, ans[l + 1] -= x;
    return make_pair(mul, res);
  }
  int mid = (l + r) >> 1;
  auto tol = F.modxk((mid - l + 1) * m);
  F = F - tol;
  for (Z x : F.a) ans[l] += x, ans[mid + 1] -= x;
  auto [ml, resl] = solve(l, mid, tol);
  F = (F * ml).divxk((mid - l + 1) * m) + resl;
  auto tor = F.modxk((r - mid) * m);
  F = F - tor;
  for (Z x : F.a) ans[mid + 1] += x, ans[r + 1] -= x;
  auto [mr, resr] = solve(mid + 1, r, tor);
  F = (F * mr).divxk((r - mid) * m) + resr;
  return make_pair(ml * mr, F);
}
...
void work() {
  ...
  rep (i, 1, n) {
    p[i].resize(2 * m + 1);
    rep (j, 0, 2 * m) cin >> p[i][j].x;
  }
  rep (i, 1, n) ans[i] = 0;
  solve(1, n, Poly({1}));
  rep (i, 1, n) ans[i] += ans[i - 1];
  rep (i, 1, n) cout << ans[i].val() << " \n"[i == n];
}
